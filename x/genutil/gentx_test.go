...
func (suite *GenTxTestSuite) SetupTest() {
	...
	suite.msg1, err = stakingtypes.NewMsgCreateValidator(
		sdk.ValAddress(pk1.Address()).String(), pk1, amount, desc, comm, one, "0x0", math.ZeroInt(), math.ZeroInt(), math.ZeroInt(), 30)
	suite.NoError(err)
	suite.msg2, err = stakingtypes.NewMsgCreateValidator(
		sdk.ValAddress(pk2.Address()).String(), pk1, amount, desc, comm, one, "0x0", math.ZeroInt(), math.ZeroInt(), math.ZeroInt(), 30)
	suite.NoError(err)
}
