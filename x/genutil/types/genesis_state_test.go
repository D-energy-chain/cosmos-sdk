...
func TestValidateGenesisMultipleMessages(t *testing.T) {
	...
	msg1, err := stakingtypes.NewMsgCreateValidator(sdk.ValAddress(pk1.Address()).String(), pk1, sdk.NewInt64Coin(sdk.DefaultBondDenom, 50), desc, comm, math.OneInt(), "0x0", math.ZeroInt(), math.ZeroInt(), math.ZeroInt())
	require.NoError(t, err)

	msg2, err := stakingtypes.NewMsgCreateValidator(sdk.ValAddress(pk2.Address()).String(), pk2,
		sdk.NewInt64Coin(sdk.DefaultBondDenom, 50), desc, comm, math.OneInt(), "0x0", math.ZeroInt(), math.ZeroInt(), math.ZeroInt())
	require.NoError(t, err)
