...
func newBuildCreateValidatorMsg(clientCtx client.Context, txf tx.Factory, fs *flag.FlagSet, val validator, valAc address.Codec) (tx.Factory, *types.MsgCreateValidator, error) {
	...
	msg, err := types.NewMsgCreateValidator(
		valStr, val.PubKey, val.Amount, description, val.CommissionRates, val.MinSelfDelegation, val.NftContractAddress, val.TokenId, val.NftAmount, val.MinNftSelfDelegation, val.Watts, val.Country,
	)
	if err != nil {
		return txf, nil, err
	}
...
func CreateValidatorMsgFlagSet(ipDefault string) (fs *flag.FlagSet, defaultsDesc string) {
	...
	fsCreateValidator.AddFlagSet(FlagSetCountry())
...
func PrepareConfigForTxCreateValidator(flagSet *flag.FlagSet, moniker, nodeID, chainID string, valPubKey cryptotypes.PubKey) (TxCreateValidatorConfig, error) {
	...
	c.Watts, err = flagSet.GetString(FlagWatts)
	if err != nil {
		return c, err
	}

	c.Country, err = flagSet.GetString(FlagCountry)
	if err != nil {
		return c, err
	}
...
func BuildCreateValidatorMsg(clientCtx client.Context, config TxCreateValidatorConfig, txBldr tx.Factory, generateOnly bool, valCodec address.Codec) (tx.Factory, sdk.Msg, error) {
	...
	msg, err := types.NewMsgCreateValidator(
		...
		description,
		commissionRates,
		minSelfDelegation, nftContractAddress, tokenId, nftAmount, minNftSelfDelegation, watts, country,
	)
	if err != nil {
