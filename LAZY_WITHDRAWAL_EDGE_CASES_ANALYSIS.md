...
## ✅ **CONFIRMED: Perfect System Design**

### **Confirmed Behavior:**
1. ✅ **NFT Offsetting**: Runs ONLY at epoch end (after reward allocation)
2. ✅ **Undelegation**: Queued and processed at epoch end
3. ✅ **Reward Allocation**: Happens BEFORE both processes

### **Result:**
**NO EDGE CASES!** All delegators receive full epoch rewards before any delegation removal.

### **Implementation for Lazy Withdrawal:**

**Simplified Hook (No Period Increment Needed):**

```go
func (h Hooks) BeforeDelegationRemoved(ctx context.Context, delAddr sdk.AccAddress, valAddr sdk.ValAddress) error {
    // Both undelegation and NFT offsetting happen at epoch end
    // Rewards are already allocated and periods already incremented
    // Simple withdrawal is sufficient!
    
    rewards, err := h.k.WithdrawDelegationRewards(ctx, delAddr, valAddr)
    if err != nil {
        h.k.Logger(ctx).Error("Failed to withdraw rewards before removal", "error", err)
        return nil  // Don't fail removal
    }
...
**No need for:**
- ❌ Period increment checks
- ❌ Mid-epoch handling
- ❌ Accumulated reward checks
- ❌ Complex edge case logic

**Why it's simple:**
Both removal paths (undelegation & NFT offsetting) run at epoch end, after allocation and period increment. The hook just needs to withdraw accumulated rewards - they're already calculated!

---

## 🎯 **Action Items**

1. ✅ **CONFIRMED**: NFT offsetting runs at epoch end (after allocation)
2. ✅ **CONFIRMED**: Undelegation processed at epoch end
3. **Implement**: Simple withdrawal hook (no edge cases!)
4. **Test**: Verify full rewards withdrawn before removal

**Ready to implement lazy withdrawal with SIMPLE, clean code!** 🚀
